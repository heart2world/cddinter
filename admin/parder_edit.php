 ...
 * 合作伙伴编辑
 *
 * @version        $Id: parder_edit.php 1 11:17 2010年7月19日Z tianya $
...
if($dopost=='del')
{
    ...
    if($ids=="")
    {
        $myrow = $dsql->GetOne("SELECT url FROM #@__parder WHERE id='".$id."'");
        ...
        {
            $msg = "成功删除一个附件！";
            $dsql->ExecuteNoneQuery("DELETE FROM #@__parder WHERE id='".$id."'");
        }
        ShowMsg($msg,'parder_list.php');
        exit();
    } else {
        $ids = explode(',', $ids);
        $idquery = "";
        ...
        $dsql->SetQuery("SELECT id,url FROM #@__parder $idquery ");
        $dsql->Execute();
        ...
        while($myrow=$dsql->GetArray())
        {
            ...
            if($rs==1)
            {
                $dsql->ExecuteNoneQuery("DELETE FROM #@__parder WHERE id='".$myrow['id']."'");
            }
        }
        ShowMsg('成功删除选定的文件！','parder_list.php');
        exit();
    }
}
/*--------------------------------
...
else if($dopost=='save')
{
    ...
    for($i=1; $i<=2; $i++)
    {
        ...
        //写入数据库
        $query = " UPDATE #@__parder SET title='$title',sort='$sort'";
        $query .= "$addquery WHERE id='$id' ";
        $dsql->ExecuteNoneQuery($query);
        ShowMsg('成功更改一则轮播图！','parder_edit.php?id='.$id);
        exit();
    }
...
//读取档案信息
$myrow = $dsql->GetOne("SELECT * FROM #@__parder WHERE id='".$id."'");
if(!is_array($myrow))
{
    ShowMsg('错误，找不到此轮播图！','javascript:;');
    exit();
}

include DedeInclude('templets/parder_edit.htm');