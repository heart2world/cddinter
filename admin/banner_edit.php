 ...
 * 轮播图编辑
 *
 * @version        $Id: banner_edit.php 1 11:17 2010年7月19日Z tianya $
...
if($dopost=='del')
{
    ...
    if($ids=="")
    {
        $myrow = $dsql->GetOne("SELECT url FROM #@__banner WHERE id='".$id."'");
        ...
        {
            $msg = "成功删除一个附件！";
            $dsql->ExecuteNoneQuery("DELETE FROM #@__banner WHERE id='".$id."'");
        }
        ShowMsg($msg,'banner_list.php');
        exit();
    } else {
        $ids = explode(',', $ids);
        $idquery = "";
        ...
        $dsql->SetQuery("SELECT id,url FROM #@__banner $idquery ");
        $dsql->Execute();
        ...
        while($myrow=$dsql->GetArray())
        {
            ...
            if($rs==1)
            {
                $dsql->ExecuteNoneQuery("DELETE FROM #@__banner WHERE id='".$myrow['id']."'");
            }
        }
        ShowMsg('成功删除选定的文件！','banner_list.php');
        exit();
    }
}
/*--------------------------------
...
else if($dopost=='save')
{
    ...
    for($i=1; $i<=2; $i++)
    {
        ...
        //写入数据库
        $query = " UPDATE #@__banner SET title='$title',sort='$sort'";
        $query .= "$addquery WHERE id='$id' ";
        $dsql->ExecuteNoneQuery($query);
        ShowMsg('成功更改一则轮播图！','banner_edit.php?id='.$id);
        exit();
    }
...
//读取档案信息
$myrow = $dsql->GetOne("SELECT * FROM #@__banner WHERE id='".$id."'");
if(!is_array($myrow))
{
...
include DedeInclude('templets/banner_edit.htm');