 ...
 * 合作伙伴添加
 *
 * @version        $Id: parder_add.php 2 15:25 2011-6-2 tianya $
...
if($dopost=="upload")
{
    ...
    for($i=0; $i<=2; $i++)
    {
        if(isset(${"upfile".$i}) && is_uploaded_file(${"upfile".$i}))
        {
            ...
            $inquery = "INSERT INTO `#@__parder`(title,url,addtime,sort)
                        VALUES ('$ntitle','$filename','$uptime','$sort'); ";
            $okdd++;
            $dsql->ExecuteNoneQuery($inquery);
        }
    }
    ShowMsg("成功上传 {$okdd} 个文件！","parder_list.php");
    exit();
}
include DedeInclude('templets/parder_add.htm');